...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < nparts; i++) {
        ROS_INFO_STREAM("orphaned  parts: " << orphan_models_wrt_world[i]
                                            << endl);
    }
    ...
    if (boxInspector.get_bad_part_Q1(current_part)) {
        ...
        // use the robot action server to acquire and dispose of the specified
        // part in the box:
    }
    ...
    // box inspector sees "model", defined in osrf_gear; convert this to our
    // datatype "Part" void model_to_part(osrf_gear::Model model,
    // inventory_msgs::Part &part, unsigned short int location)
    model_to_part(orphan_models_wrt_world[0], current_part,
                  inventory_msgs::Part::QUALITY_SENSOR_1);

    // use the robot action server to acquire and dispose of the specified part
    // in the box:
    ...
    // here's an oddity: this node runs to completion.  But sometimes, Linux
    // complains bitterly about
    // *** Error in `/home/wyatt/ros_ws/devel/lib/shipment_filler/unload_box':
    // corrupted size vs. prev_size: 0x000000000227c7c0 *** don't know why.  But
    // does not seem to matter.  If anyone figures this  out, please let me
    // know.
}
